...
public:
   ...
   */
  virtual bool init(int fd, unique_ptr<Session> session, const string &addr);

  /**
   * @brief 监听到有新的数据到达，调用此函数进行接收消息
   * 如果需要创建新的任务来处理，那么就创建一个SessionEvent 对象并通过event参数返回。
   */
  virtual bool read_event(SessionEvent *&event) = 0;
   ...
   *         是否需要断开连接
   */
  virtual bool write_result(SessionEvent *event, bool &need_disconnect) = 0;
